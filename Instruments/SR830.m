classdef SR830
    ...
    methods
        ...
        function increase_time_constant(obj)
            current_tc = str2num(obj.dev.query('OFLT ?'));
            if current_tc < 19
        ...
        function [result]=increase_sensitivity(obj)
            current_s = str2num(obj.dev.query('SENS ?'));
            if current_s < 26
        ...
        function [ref] = get_ref_freq(obj)
            ref = str2num(obj.dev.query('FREQ?'));
        end
                
        function [sens] = get_sens(obj)
            sens = str2num(obj.dev.query('SENS?'));
        end
        ...
        function [phase] = get_phase(obj)
            phase = str2num(obj.dev.query('PHAS?'));
        end
        ...
        function [oflt] = get_oflt(obj)
            oflt = str2num(obj.dev.query('OFLT?'));
        end
                
        function [numeric,str_value]=get_sensitivity(obj)
            [numeric,str_value]=obj.convert_sens_to_value(str2num(obj.dev.query('SENS?')));
        end
        
        function [numeric,str_value]=get_time_constant(obj)
            [numeric,str_value]=obj.convert_tc_to_value(str2num(obj.dev.query('OFLT?')));
        end
